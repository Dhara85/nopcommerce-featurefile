Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  Scenario: Verify thst user can navigate to Books category
    When  User is on given URL
    And   Click on Books tab on Top menu
    Then  User is navigated to Books Category page

  Scenario: Verify that uset can see the Books Category page with filters and list of products
    When  User is on given URL
    And   Click on Books tab on Top menu
  ...
  Scenario: Verify that user can see 'Name: A to Z' filter is functioning
